 ...
 * appropriately.
 */
int open_with_extention(const char *base, const char *extention, int flags)
{
    ...
    buf = malloc(base_len + extention_len + 1);
    if (!buf) {
        errno = -ENOMEM;
        return -1;
    }
    ...
    strncpy(buf + base_len, extention, extention_len);
    buf[base_len + extention_len] = '\0';
 ...
 * error
 */
int load_buffer(const char *base, const char *extention,
                char *buf, size_t len)
{
    int fd;
    int rc;

    fd = open_with_extention(base, extention, O_RDONLY);
    if (fd == -1)
        return -errno;
    ...
    if (rc == -1) {
        rc = -errno;
        error(rc, "Failed to read %lu bytes from '%s%s'", len, base, extention);
    }
    close(fd);
...
int dump_buffer(const char *base, const char *extention,
                char *buf, size_t len)
{
    int fd;
    int rc;

    fd = open_with_extention(base, extention, O_WRONLY | O_CREAT);
    if (fd == -1)
        return -errno;
    ...
    if (rc == -1) {
        rc = -errno;
        error(rc, "Failed to write %lu bytes to '%s%s'", len, base, extention);
    } else {

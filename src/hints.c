...
char *get_key_value(char *input, char **key, char **value)
{
    ...
    *value = NULL;
    ...
    if (!end || (end == input)) {
        if (end == input) {
            ...
            pho_error(-EINVAL, "failed to parse hint '%s', missing '='",
                      input);
        }
        errno = EINVAL;
        return NULL;
    ...
    *end++ = '\0';
    ...
    if (end == input) {
        pho_error(-EINVAL, "failed to parse hint '%s', missing value", input);
        errno = EINVAL;
        return NULL;
    }
...
int process_hints(const struct buf *hints, struct hinttab *hinttab)
{
    ...
    do {
        ...
        pho_info("hint: '%s' (k: %s, v: %s)", hint, key, value);
        if (!errno && key && value) {

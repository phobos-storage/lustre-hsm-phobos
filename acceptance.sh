...
function check_layout_component()
{
    ...
    user_md_contains "$oid" "layout_comp$id" \
        "extent_start=$(lfs getstripe --component-id="$id" --component-start "$file")" ||
        invalid_component_error "$file" "$id"

    user_md_contains "$oid" "layout_comp$id" \
        "extent_end=$(lfs getstripe --component-id="$id" --component-end "$file")" ||
        invalid_component_error "$file" "$id"
}
...
function check_layout_pfl()
{
    ...
    do
        user_md_contains "$oid" "layout_comp$i" "stripe_count" ||
            error "'layout_comp$i' not in found user_md of '$oid'"
...
function test_archive_failure()
{
    ...
    phobos dir lock "$ARCHIVE_PATH"
    trap_add "phobos dir unlock '$ARCHIVE_PATH'"
    ...
    # Make sure that the request finally succeed
    phobos dir unlock "$ARCHIVE_PATH"
    lfs hsm_archive "$file"
    wait_for_event ARCHIVE_FINISH "$file"

    pgrep lhsmtool_phobos
    cat "$EVENTS"
}
add_test archive_failure

...
static void usage(const char *name, int rc)
{
    fprintf(stdout,
            ...
            "    -l, --restore-lov            Use the strping that the file "
...
/* FIXME the layout API doesn't provide a function to restored \p layout
...
static int ct_archive(const struct hsm_action_item *hai, const long hal_flags)
{
    ...
    if (!layout)
        CT_ERROR(rc, "cannot read layout of '%s'", src);
